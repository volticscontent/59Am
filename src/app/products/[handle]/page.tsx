...
export default function ProductPage() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      const fetchedProductId = product.id ? parseInt(product.id.toString().replace(/\D/g, ''), 10) : 0;

      // Fetch precise details from our PostgreSQL database API just to ensure we have the right IDs/Prices
      const res = await fetch(`/api/products/${fetchedProductId}`);
      if (!res.ok) {
        console.error('Produto não encontrado no banco de dados:', fetchedProductId);
        return;
      }
      const dbProduct = await res.json();

      const variantId = dbProduct.data?.variants?.[0]?.id ? parseInt(dbProduct.data.variants[0].id.toString().replace(/\\D/g, ''), 10) : 0;
  ...
  useEffect(() => {
    if (product) {
      trackStoreProductViewed(product as unknown as any);
    }
  }, [product, trackStoreProductViewed]);
              ...
              <div className="flex gap-2 justify-center">
                {product.images.map((image, index) => (
                  <button
                    ...
                    className={`relative w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${selectedImage === image ? 'border-white' : 'border-white/30'
                      }`}
                  >
                    <Image
                      src={image}
